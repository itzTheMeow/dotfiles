...
- **Impure nixpkgs overlays** (`nixos/common.nix`): `nix.nixPath` adds `nixpkgs-overlays=/etc/nix/nixpkgs-overlays`, whose files (an `environment.etc` dir) are one thin `import "<xelib.location>/overlays/<name>"` wrapper per `overlays/*.nix` plus `zz-unstable.nix` (a bare `unstable` from `<nixpkgs-unstable>`). This is the only way `nix-shell -p` / `nix-build` / `nix-env` see repo overlays — they build their package set from `NIX_PATH`, and the flake's overlays never apply there. nixpkgs loads it only in _impure_ evals, and pure eval can't read NIX*PATH at all, so flakes/`nixos-rebuild`/CI are unaffected. Deploy gotcha: after `nixos-rebuild switch`, existing sessions/tmux keep the old NIX_PATH because `/etc/zshenv`+`/etc/profile` skip `/etc/set-environment` when the inherited `__NIXOS_SET_ENVIRONMENT_DONE=1` marker is set — verify in a truly fresh login (`env -u __NIXOS_SET_ENVIRONMENT_DONE zsh -l`), not just a new tab/window. Two constraints: it must be one file per overlay (nixpkgs `throwIfNot`s that every entry in the dir is a \_function*; folding the list into one `final: prev: ...` breaks the stdenv booter with an infinite recursion, and `lib.mkMerge` doesn't unwrap either — so don't try to consolidate the dir), and the `unstable` import must pass `overlays = [ ]` (otherwise nixpkgs re-applies the same dir inside the unstable instance and `unstable.siyuan` recurses forever via `overlays/siyuan.nix`). `unstable` there is upstream-only, so `unstable.<custom overlay pkg>` is _not_ available. The 3 flake-input overlays (copyparty, nix-vscode-extensions, timefinder-electron) are also absent there. Don't combine with `~/.config/nixpkgs/overlays` — `nixpkgs-overlays` wins and nixpkgs errors if both the home `overlays.nix` file and `overlays/` dir exist.

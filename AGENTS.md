...
- **SSH client config**: per-user `~/.ssh/config` is managed only by home-manager `programs.ssh` (NixOS `programs.ssh` configures just system-wide `/etc/ssh/ssh_config`: known hosts, package, defaults). `programs.ssh.matchBlocks` (and its `extraOptions`) is **deprecated** — use `programs.ssh.settings."<host>"` with CamelCase ssh_config(5) directives (`HostName`, `User`, `Port`, `IdentityFile`, `IdentitiesOnly`, `ForwardAgent`...), `settings."*"` for the default block, and `hm.lib.hm.dag` (in nixos-tree modules) to order blocks. `mkSSHConfig` in `xelib` emits these from the host table. The ssh **key sops secrets are declared on the NixOS side** (`sops.opSecrets.ssh_pubkeys`/`git_ssh` via `mkSSHSecrets` + the code-host secret blocks in `_common/git.nix`), so `IdentityFile` points at `/run/secrets/<name>` — the settings themselves stay HM-side. Machine blocks and code-host blocks are each emitted with `hm.lib.hm.dag.entryBefore [ "*" ]` (see `mkSSHConfig` in `xelib` and `_common/git.nix`), so the `"*"` block in `_common/ssh.nix` renders last with no after-list needed.
